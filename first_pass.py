...
def first_pass(lines):
    ...
    for line in lines:
        ...
        if "," in line and (len(label) > 3 or label == "I"):
            print(f"Invalid label: {label} at line {cnt}.")
            print("Exiting...")
            exit()
            
        if "ORG" in label:
            lc = format(int(label.split(" ")[1], 16), "03X")
            continue

        if len(label) <= 3:
            if (
                not (label in reg_instructions)
                and not (label in pseudo_instructions)
                and not (label in mem_instructions)
            ):
                if label in symbol_table:
                    print(f"Duplicate label {label} at line {cnt}")
                    print("Exiting...")
                    exit()
